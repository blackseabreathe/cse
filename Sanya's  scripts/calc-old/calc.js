	...
	geography: function(elem){
		...
		$.ajax({
			...
			success: function(options){
				...
				$('.preloader').remove();
				calc.select_value();
				// здесь скрипты для удаления города и страны
				if(elem.attr('name') == 'country' || elem.attr('name') == 'country2') $("select option:contains('Москва г')").prop('selected', true).trigger('change');
			}
		});
	}
